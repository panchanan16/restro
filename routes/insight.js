...
route.post("/getByPeriod", (req, res)=>{
    ...
    conn.query(`SELECT menu_items.item_name, orders_.* FROM orders_ JOIN menu_items ON orders_.item_id = menu_items.item_id WHERE order_date between date_sub('${req.body.start}',INTERVAL ${req.body.end} DAY) and '${req.body.start}';  SELECT order_approved,  COUNT(*) AS total FROM orders_ WHERE order_date between date_sub('${req.body.start}',INTERVAL ${req.body.end} DAY) and '${req.body.start}' GROUP BY order_approved;`, (err, result)=>{
